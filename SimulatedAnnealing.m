function SimulatedAnnealing()
    ...
    state2 = OptimiseRoute2(cities, initialTemperature, endTemperature); % call our optimization function

    figure
    plot(cities(state2,1),cities(state2,2),"g--o"); % draw final route
    title('Opt2 route')

    state3 = OptimiseRoute3(cities, initialTemperature, endTemperature); % call our optimization function
    
    figure
    plot(cities(state3,1),cities(state3,2),"g--o"); % draw final route
    title('Opt3 route')
...
function [ state ] = OptimiseRoute2(cities, initialTemperature, endTemperature)
    nCities = size(cities,1);
    state = [1:nCities]'; % setup initial cities visit order as numbered column-vector (transposed row)
    currentEnergy = CalculateEnergy(state, cities); % calculate the energy for the initial condition
    T = initialTemperature;
    ...
    for k = 1:100000 % main loop
        stateCandidate = GenerateStateCandidate2(state); % create a new order for visiting cities
        candidateEnergy = CalculateEnergy(stateCandidate, cities); % calculate its energy
    ...
    disp('Final 2 route length: ');
    disp(currentEnergy);
end

function [ state ] = OptimiseRoute3(cities, initialTemperature, endTemperature)
    nCities = size(cities,1);
    state = [1:nCities]'; % setup initial cities visit order as numbered column-vector (transposed row)
    currentEnergy = CalculateEnergy(state, cities); % calculate the energy for the initial condition
    T = initialTemperature;
    ...
    for k = 1:100000 % main loop
        stateCandidate = GenerateStateCandidate3(state); % create a new order for visiting cities
        candidateEnergy = CalculateEnergy(stateCandidate, cities); % calculate its energy
    ...
    disp('Final 3 route length: ');
    disp(currentEnergy);
end

function [ seq ] = GenerateStateCandidate2(seq)
   n = size(seq, 1); % get size of cities indexes array
   i = randi(n, 1);
   j = randi(n, 1);
   k = (i + j)/2;
   ...
   g = max(i,j);
   for l = h:k
   t = seq(l); % swap 2 indexes
   seq(l) = seq(g-(l-h));
   seq(g-(l-h)) = t;
   end
end

function [ seq ] = GenerateStateCandidate3(seq)
   n = size(seq, 1); % get size of cities indexes array
   i = randi(n, 1);
   j = randi(n, 1);
   h = min(i,j);
